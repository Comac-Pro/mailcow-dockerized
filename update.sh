...
while (($#)); do
  case "${1}" in
    --check|-c)
      ...
      LATEST_REV=$(git ls-remote --exit-code --refs --quiet https://github.com/Comac-Pro/mailcow-dockerized "${BRANCH}" | cut -f1)
      if [ "$?" -ne 0 ]; then
        echo "A problem occurred while trying to fetch the latest revision from github."
        exit 99
      fi
      if [[ -z $(git log HEAD --pretty=format:"%H" | grep "${LATEST_REV}") ]]; then
        echo -e "Updated code is available.\nThe changes can be found here: https://github.com/Comac-Pro/mailcow-dockerized/commits/master"
    ...
    --check-tags)
      ...
      if [[ -z $(git log HEAD --pretty=format:"%H" | grep "${LATEST_TAG_REV}") ]]; then
        echo -e "New tag is available.\nThe changes can be found here: https://github.com/Comac-Pro/mailcow-dockerized/releases/latest"
        exit 0
      else
        echo "No updates available."
        exit 3
...
if [ ${#DOTS} -lt 1 ]; then
  ...
  echo "Find more information about why this message exists here: https://github.com/Comac-Pro/mailcow-dockerized/issues/1572"
...
if [ ! "$DEV" ]; then
  DEFAULT_REPO="https://github.com/Comac-Pro/mailcow-dockerized"
...
if [ $? -eq 0 ]; then
  ...
  echo '  $MAILCOW_GIT_URL="https://github.com/Comac-Pro/mailcow-dockerized";' >> data/web/inc/app_info.inc.php
